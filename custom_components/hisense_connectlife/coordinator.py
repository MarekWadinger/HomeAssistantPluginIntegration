...
class HisenseACPluginDataUpdateCoordinator(
    ...
    async def async_refresh_device(self, device_id: str) -> None:
        ...
        try:
            ...
            if devices and device_id in devices:
                ...
                _LOGGER.warning(
                    "Device %s not found in update response", device_id
                )
    ...
    async def async_control_device(
        ...
        try:
            _LOGGER.debug(
                "Controlling device %s with properties: %s", puid, properties
            )
    ...
    def _handle_ws_message(self, message: dict[str, Any]) -> None:
        ...
        try:
            ...
            try:
                ...
                for key, dev in self._devices.items():
                    if dev.puid == device_id:
                        ...
                        device_key = key
                        _LOGGER.debug(
                            "Found device with puid %s, device_id: %s",
                            device_id,
                            key,
                        )
                        break
                ...
                if device and device_key:
                    ...
                    if msg_type == "status_wifistatus":
                        ...
                        if status and isinstance(status, str):
                            try:
                                decoded_status = base64.b64decode(
                                    status
                                ).decode("utf-8")
                                status_json = json.loads(decoded_status)
                                _LOGGER.debug(
                                    "Decoded status: %s", status_json
                                )
                                if status_json and isinstance(
                                    status_json, dict
                                ):
                        ...
                        if properties and isinstance(properties, dict):
                            for key, value in properties.items():
                                device_data["statusList"][key] = value
                            _LOGGER.debug(
                                "Updated device status with properties: %s",
                                properties,
                            )
                    ...
                    _LOGGER.debug(
                        "Device with puid %s not found in devices list",
                        device_id,
                    )
            ...
            _LOGGER.error(
                "Error handling websocket message: %s", error, exc_info=True
            )
