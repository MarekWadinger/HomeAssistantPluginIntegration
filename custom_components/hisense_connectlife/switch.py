...
async def async_setup_entry(
    ...
    try:
        ...
        for device_id, device in devices.items():
            _LOGGER.debug(
                "Processing device for switches: %s", device.to_dict()
            )
            ...
            if isinstance(device, HisenseDeviceInfo) and device.is_devices():
                ...
                if device.type_code == "007":
                    ...
                    if parser and "t_fan_speed" in parser.attributes:
                        _LOGGER.info(
                            "除湿机添加风速进入: %s", device.feature_code
                        )
...
class HisenseSwitch(CoordinatorEntity, SwitchEntity):
    ...
    def available(self) -> bool:
        ...
        if self._switch_type == "rapid_mode":
            ...
            if current_mode in [
                "4",
                "3",
            ]:  # Assuming "0" is AUTO and "1" is FAN
                return False
    ...
    def is_on(self) -> bool:
        ...
        if current_time - self._last_action_time < self._debounce_delay:
            _LOGGER.info(
                "当前处于防跳变时间窗口内，使用缓存状态: %s", self.cached
            )
            return self.cached
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        self._last_cloud_value = (
            None  # 重置云端状态值（因为即将发送新的控制指令）
        )
